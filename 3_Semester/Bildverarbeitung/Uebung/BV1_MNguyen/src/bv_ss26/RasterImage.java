	...
	public void convertToGray() {
		...
		for (int y = 0; y < height; y++){
			for (int x = 0; x < width; x++){
				int pos = y * width + x;
				int pixel = argb[pos];
				int r = (pixel >> 16) & 0xff; //um nur den den roten Wert zu bekommen müssen die Werte um 16 Bit verschoben werden + Bitweises & + Maske damit die anderen Farbwerte verschwinden
				int g = (pixel >> 8) & 0xff;
				int b = pixel & 0xff;
				int gray = (r + g + b) / 3;
