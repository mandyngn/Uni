...
public class Matrix4 {
	...
	public Matrix4(float near, float far, float fovYRadians, float aspect) {
		// Eigene Initialisierung statt this(), damit wirklich nur die Projektionswerte gesetzt sind.
		matrix = new float[4][4];

		float f = 1.0f / (float) Math.tan(fovYRadians / 2.0f);
		float rangeInv = 1.0f / (near - far);

		// OpenGL-kompatible Perspektivprojektion (spaltenweise Weitergabe erfolgt in getValuesAsArray()).
		matrix[0][0] = f / aspect;
		matrix[1][1] = f;
		matrix[2][2] = (far + near) * rangeInv;
		// Wichtig: fuer OpenGL muss -1 in Zeile 4, Spalte 3 stehen (Index [3][2]).
